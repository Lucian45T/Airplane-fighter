const positionBulletintoPlane = 48;
const positionExplosionSwapPlanePlace = 25;
const speedFallObstacle = 10;
const speedBullet = 10
const maxUpBulletPosition = - 10;
const initialObstacleVerticalPosition = - 30;
...
let bulletX = planeX + positionBulletintoPlane;
let bulletY = planeY;
let explosionX = planeX + positionExplosionSwapPlanePlace;
let explosionY = planeY + positionExplosionSwapPlanePlace;

var obstaclePositions = [35, 105, 180, 
  250, 320, 390, 435];
  
var differentObstaclesSpeedFall = [10, 15, 25, 20, 7];
...
document.addEventListener("keydown", (e) => {
  if (hit === 0) {
    if (e.key === "ArrowRight" && planeX <= maxRightPosition) planeX += speedFallObstacle; 
    if (e.key === "ArrowLeft" && planeX >= maxLeftPosition)  planeX -= speedFallObstacle;
    if (e.key === "ArrowUp" && planeY >= maxUpPosition) planeY -= speedFallObstacle;
    if (e.key === "ArrowDown" && planeY <= maxDownPosition)  planeY += speedFallObstacle;
...
function bulletReset() {
  bulletX = planeX + positionBulletintoPlane;
  bulletY = planeY;
}  
...
function fire() {
  ...
  if (bulletY == maxUpBulletPosition) { 
...
function obstacleRandomPosition() { 
  ...
  obstacleDiv.style.top = initialObstacleVerticalPosition + "px";
  let obstacleY = {value: initialObstacleVerticalPosition};
  document.getElementById("container").appendChild(obstacleDiv);
  let delay = Math.floor(Math.random() * differentObstaclesSpeedFall.length);
  let differentSpeedFall = differentObstaclesSpeedFall[delay];
...
function collisionObstacles(element1, element2) {
  ...
  if (collided) {
    ...
    explosionX = planeX + positionExplosionSwapPlanePlace;
    explosionY = planeY + positionExplosionSwapPlanePlace;
    explosionId.style.left = explosionX + "px";
    explosionId.style.top = explosionY + "px";
    explosionCollision();
    ++hit;
